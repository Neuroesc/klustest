function klustfig_part(pdata,sdata,pp,wav_now,quals,fets,config)
% klustfig_part figure function for klustest
% Data visualisation function for klustest, given processed data tables and some
% parameters, creates a summary figure for every cluster, in each part.
%
% USAGE
%
% klustfig_part(pdata,sdata,pp,wav_now,quals,fets,config)
%
% INPUT
%
% 'pdata' - Table, one of the main outputs from klustest, part data, such as 
%       position data and LFP 
%
% 'sdata' - Table, one of the main outputs from klustest, session data, such as
%       spikes and firing rates
%
% 'pp' - Scalar, the part we want to plot right now
%
% 'wav' - Cell array, waveforms for all spikes, each cell is an electrode
%       (samples x channels x spikes), given by get_spk_for_klustest
%
% 'quals' - Cell array, feature space histograms
%           (bin x-values; within cluster distances; between cluster distances)
%           For every cluster, cells correspond to electrodes, provided by 
%           get_iso_for_klustest
%
% 'fets' - Cell array, spike features for all spikes, cells correspond to
%           electrodes, provided by get_iso_for_klustest
%
% 'config' - Structure, configuration file from klustest, contains the
%           input/output filename to check for tetrodes etc
%
% NOTES
% 1. Function is not really intended to be used without klustest
% 
% SEE ALSO klustest
...
% version 3.0.1, Release 18/02/23 Added limits to the number of waveforms and features plotted to prevent crashing
% version 3.0.2, Release 18/02/23 Added text to show the N spikes and features displayed
% version 3.0.3, Release 18/02/23 Using plot for spikes instead of scatter, faster and looks better I think
% version 3.0.4, Release 29/11/25 Updates for GitHub release
% version 3.0.5, Release 16/12/25 updated filenames for cross platform flexibility
% version 3.0.6, Release 16/12/25 added config input for easier parameter definitions
% version 3.0.7, Release 16/12/25 improved comments, not ideal but detailed enough for now
%
% AUTHOR 
% Roddy Grieves
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% INPUTS
%%%%%%%%%%%%%%%% ARGUMENT CHECK
    fig_clust = figure('visible',config.show_figs,'Units','pixels','Position',[10, 10, 1600, 900]); % open/create figure
    ...
    % settings
    max_plot_waves = config.n_waveforms; % set to 0 to show all waveforms
    max_plot_spikes = config.max_plot_spikes; % set to 0 to show all spikes in spike & position plot
    tets = pdata.tetrodes;
    mapset = pdata.mapset;
...
%%%%%%%%%%%%%%%% FIRING RATE MAP    
    axrt = axes('Units','pixels','Position',[axps.Position(1) axps.Position(2)-axps.Position(4)-55 axps.Position(3) axps.Position(4)]);
        ...
        if config.spatial_shuffles % spatial shuffles were performed
...
%%%%%%%%%%%%%%%% CLUSTER QUALITY
        axiso = axes('Units','pixels','Position',[axwav.Position(1)+20 axwav.Position(2)-axwav.Position(4)-55 axwav.Position(3)+30 axwav.Position(4)]);
            ...
            if ~isempty(dists)
                ...
                b1 = area(dists(1,:),h1,'FaceColor','b','FaceAlpha',0.5,'EdgeColor','none'); hold on;
                b2 = area(dists(1,:),h2,'FaceColor','k','FaceAlpha',0.5,'EdgeColor','none');     
...
%%%%%%%%%%%%%%%% SPIKE AUTOCORRELOGRAM - 25MS REFRACTORY PERIOD            
...
%%%%%%%%%%%%%%%% SPIKE AUTOCORRELOGRAM - 500MS THETA MODULATION         
...
%%%%%%%%%%%%%%%% SAVE FIGURE
    % Save the figure  
    foname = fullfile(pwd,pdata.outname,'part_figures');
    [~,~,~] = mkdir(foname); % create a folder to hold outputs 
    fname = fullfile(foname,[sdata.uci{1} '_' part_now '.png']);
    warning('off','MATLAB:Axes:NegativeLimitsInLogAxis');
    if config.fast_figures
        ...
        exportgraphics(fig_clust,fname,'BackgroundColor',[1 1 1],'Colorspace','rgb','Resolution',config.fig_res);  
    end
    close(fig_clust);    
