function [ratemap,dwellmap,spikemap,config,mapset] = mapDATA5(pos,spk,config)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%     [r,d,s,o] = mapDATA5(pos,spk,'dwellmap',d);
%     subplot(2,2,3)
%     imagesc(r);
%     daspect([1 1 1])
% 
%     % make a new map with different settings
%     [r,d,s,o] = mapDATA5(pos,spk,'bin_size',5,'map_sigma',3);
%     subplot(2,2,4)
%     imagesc(r);
%     daspect([1 1 1])
%
...
% version 5.0.0, Release 31/10/17 changed to v5, overhauled, simplified inputs, removed redundant code
%
...
% validData = @(x) ismatrix(x) && ~all(isnan(x(:))) && ~isempty(x) && isnumeric(x(:));
% validString = @(x) isstring(x) || ischar(x) && ~isempty(x);
% p = inputParser;
% p.addRequired('pos',validData);
% p.addRequired('spk',validData);
% p.addOptional('dwellmap',[]);
% p.addOptional('rmethod','nearest',validString);
% p.addOptional('smethod',1,@isscalar);
% p.addOptional('map_padd',2,@isscalar);
% p.addOptional('map_lims',[],validData);
% p.addOptional('bin_size',2,@isscalar);
% p.addOptional('map_sigma',1.5,@isscalar);
% p.addOptional('min_dwell',0.1,@isscalar);
% p.addOptional('g_sigma',10,@isscalar);
% p.addOptional('min_dist',1,@isscalar);
% p.addOptional('max_dist',3,@isscalar);
% p.addOptional('srate',50,@isscalar);
% p.parse(pos,spk,varargin{:});
% 
% dwellmap = p.Results.dwellmap;
% rmethod = p.Results.rmethod;
% smethod = p.Results.smethod;
% padd = p.Results.map_padd;
% map_lims = p.Results.map_lims;
% bin_size = p.Results.bin_size;
% map_sigma = p.Results.map_sigma;
% min_dwell = p.Results.min_dwell;
% g_sigma = p.Results.g_sigma;
% min_dist = p.Results.min_dist;
% max_dist = p.Results.max_dist;
% srate = p.Results.srate;
% output = struct;
% output.map_configuration = p.Results;
