function mtint = getDACQDATA(cname,snames,tetrodes)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
% read .inp files and accumulate their data
    ...
    if all(inpPresent)
        for pp = 1:numel(fileStruct)      
            if exist([snames{pp} '.txt'],'file') % if a .key file already exists
                [count,timestamps,spec,value,text_value] = saveKEY([snames{pp} '.inp'],2); % load it
            else
                [count,timestamps,spec,value,text_value] = saveKEY([snames{pp} '.inp'],1); % if not then make one     
            end           
            ...
            inps.text_value = [inps.text_value; text_value(:)];        
    ...
    for ifile = 1:numel(eegfilelist)
        ...
        if all(eegPresent)
            for sn = 1:length(snames)
                ...
                lfp(ifile).lfp = [lfp(ifile).lfp; int8(lfpv)]; % concatenate eeg
