...
[fname,pname,ftindx] = uigetfile('*.*','Select a file with the appropriate filename...');
...
% get the 3D reconstructed position data for the whole session
parentpath = cd(cd('..'));
fcheck = [parentpath '\3D matfiles\' cname '_merge_tracking.mat'];
if exist(fcheck,'file')
    load(fcheck);
else
...
%% sort out spike data
            ...
            spkindx = knnsearch(apot,spt); % get an index of position data for each spike (i.e. which pox,poy,poz value most closely corresponds to each spike)    
            spx = pox(spkindx);
            spy = poy(spkindx);            
            spz = poz(spkindx);            
            ...
            for gg = 1:length(part_indx) % for each specified goal
                ...
                for tt = 1:length(goal_times(:,1)) % for every trial associated with this goal
                    ...
                    gspk = [gspk; tspx(:) tspy(:) tspz() tspt(:) ones(size(tspx(:))).*traj_now];
                    sindax = [sindax; sindx(:)];
...
ppm = 1000;
sampr = 50;
[ratemap,dwellmap] = Ratemap3D(pox,poy,poz,spx,spy,spz,binsize,min_dwell_dist,ppm,sampr);                  
%% smooth ratemap
intersmoo = 3;
rsize = 0;
if intersmoo > 0
	indx = isnan(ratemap);
	ratemap(indx) = 0;
	ratemap = smooth3(ratemap,'gaussian',[intersmoo intersmoo intersmoo]);									% smooth the array in 3 bin blocks using a gaussian
	dwellmap = smooth3(dwellmap,'gaussian',[intersmoo intersmoo intersmoo]);								% smooth the array in 3 bin blocks using a gaussian
	ratemap(indx) = NaN;
end % if intersmoo > 0

%% resize ratemap
if rsize > 0
	ratemap = resize(ratemap,size(ratemap) * rsize);											% increase the resolution of the firing rate block by factor rsize
	dwellmap = resize(dwellmap,size(dwellmap) * rsize);											% increase the resolution of the firing rate block by factor rsize
	%rate_array = interp3(rate_array,2);														% increase the resolution of the firing rate block
	%time_arraySI = interp3(time_arraySI,2);													% increase the resolution of the firing rate block
end % if rsize > 0

figure
h = vol3d('cdata',rot90(ratemap,3),'texture','3D');
view(3);
alphamap('rampup');
alphamap(0.4 .* alphamap);
colormap('jet');
%colorbar('SouthOutside');
caxis([0 max(max(max(ratemap)))]);
