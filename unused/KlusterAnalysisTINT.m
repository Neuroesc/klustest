...
part_names = {'Sleep box','Square box'}; % set to empty to just use the session names
...
%% Individual cell figure
                if spike_fig
                    ...
                    ann_str = sprintf('Tetrode: %d, Cluster: %d, Spikes: %d, Time: %d, Frate: %.2f, Analysed: %s',tet,clu,n_spikes,duration,frate,datestr(now,'yyyy-mm-dd-HH-MM-SS'));
                    ...
                    if ~ignore_pos
                        ...
                        axis xy off
                        set(gca,'LineWidth',flw,'layer','top','FontSize',fsiz); 
                        title(sprintf('%.2fs (%.2f mins)',duration,duration/60));  
                    ...
                    %% Save the figure        
                    id = [cname '_E' num2str(tet) '_C' num2str(clu) '_' c_type];
                    set(gcf,'visible','off')
                    print(fig_cell,'-dpng','-r150',['Figures\' id '.png'])
                    %saveas(fig_cell,['Figures\' id '.png'],fig_format);
                    close(fig_cell);
