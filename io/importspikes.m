...
spikestruct = struct('timestamp1',0,'waveform1',int8(zeros(samples_per_spike,1)), ...
                     'timestamp2',0,'waveform2',int8(zeros(samples_per_spike,1)), ...
                     'timestamp3',0,'waveform3',int8(zeros(samples_per_spike,1)), ...
                     'timestamp4',0,'waveform4',int8(zeros(samples_per_spike,1)));
...
for ii = 1:num_spikes
   ...
   spikes(ii).waveform1 =  int8(sum(w1_bytes .* little_endian_matrix, 1)); % signals are little-endian
   spikes(ii).waveform2 =  int8(sum(w2_bytes .* little_endian_matrix, 1));
   spikes(ii).waveform3 =  int8(sum(w3_bytes .* little_endian_matrix, 1));
   spikes(ii).waveform4 =  int8(sum(w4_bytes .* little_endian_matrix, 1));
end
