...
close all
fclose('all');
out_name = 'kwiktint'; % the output name used by kwiktint when combining files
...
part_names = {'square'};
part_ignore = [1]; % if part_method above is 2, this should be a list of 1's, the same length as part_names (1 for analyse, 0 for ignore)
% part_names = {'square','lattice'}; % the output names for each partition (i.e. the name used when saving figures for each partition)
...
disp(sprintf('\t...will partition outputs with names: '));
for i = 1:length(part_names)
    disp(sprintf('\b%s ',part_names{i}));
end % for i = 1:length(part_names)
disp(sprintf('\t...partition method =  %d',part_method));
disp(sprintf('\t...will use a ratemap binsize of: %d cm',bin_size));
disp(sprintf('\t...and a smoothing sigma of: %d',map_sigma));
if cross_fig
    disp(sprintf('\t...will produce cluster autocorrelogram figures'));
end % if cross_fig
if clus_fig
    disp(sprintf('\t...will produce cluster space figures'));
end % if clus_fig
if spike_fig
    disp(sprintf('\t...will produce individual cluster figures'));
end % if spike_fig
if mtint_override
    disp(sprintf('\tWARNING: will override any existing mtint file'));
end % if combine
...
setdirs = dir('*.set');
if numel(setdirs) == 1
    setname = setdirs.name;
    disp(sprintf('...only 1 .set file detected: %s',setname))
    [~,nme,~] = fileparts(setname);
    snames = {nme};
    cname = nme;    
    nsess = 1;
else
    ...
    if numel(snames) > 1
        cname = out_name;
    end % if numel(snames) > 1
end % if numel(setdirs) == 1
...
[~,~,~] = mkdir([pwd '\klustest\figures']);
[~,~,~] = mkdir([pwd '\klustest\data']);
disp(sprintf('\t...done'))
...
if ~exist([pwd '\klustest\' cname '_mtint.mat'],'file') || mtint_override
	...
	save([pwd '\klustest\' cname '_mtint.mat'],'cname','mtint','-v7.3');
else
    disp(sprintf('\t...loading saved mtint'));
	load([pwd '\klustest\' cname '_mtint.mat'],'-mat');
...
for e = 1:length(tetrodes) 
    ...
    if isnan(mtint.tetrode(tet).nspike_cut)
        disp(sprintf('\t\t...grounded, skipping'));
        continue
    end % if isnan(spik_count)
...
%% If there are clusters    
    if clus_count > 0
        ...
        [isods,lratios,nds,cds,fdata,nfets] = clusterQUALITY(cname,tet,clusters);           
...
%% Spike phase analysis
                    ...
                    [phase_out,~,~] = Phase([t lfpf],double(spt));
                    spp = phase_out(:,2);
...
%% Save the figure        
                    ...
                    set(gcf,'visible','off')
                    print(fig_cell,'-dpng','-r150',[pwd '\klustest\figures\' id '.png'])
                    close(fig_cell);                
...
%% Cluster cross-correlation figure
        if cross_fig
            if clus_count > 1
                ...
                print(fig_corr,'-dpng','-r300',[pwd '\klustest\Figures\' id '.png'])
                close(fig_corr);
            end % if clus_count > 1
        end % if cross_fig
...
%% Cluster space figure
        if clus_fig
            ...
            print(fig_clu,'-dpng','-r300',[pwd '\klustest\figures\' id '.png'])
            close(fig_clu);
        end % if clus_fig
    else
        disp(sprintf('\t\t\t\t...skipping'));
...
save([pwd '\klustest\data\' cname '_sdata.mat'],'sdata','-v7.3'); % save session data
save([pwd '\klustest\data\' cname '_mtint.mat'],'mtint','-v7.3'); % save mtint file
