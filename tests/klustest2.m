...
[~,~,~] = mkdir([pwd '\klustest\' sdata.combined_name]);
disp(sprintf('\t...done'))
...
%% Generate ratemap and dwellmap    
            config.pos_ppm = pppm;
            config.spk_ppm = pspm;
            if isfield(sdata.(uci).(part_now),'ratemap_data')
                [ratemap,mapdata] = mapDATA4([ppox ppoy],[pspx pspy],sdata.(uci).(part_now).ratemap_data);
            else
            ...
            % accumulate data
            sdata.(uci).(part_now).dwellmap = dwellmap; % add data to structure            
            sdata.(uci).(part_now).ratemap = ratemap; % add data to structure
            sdata.(uci).(part_now).ratemap_data = mapdata; % add data to structure
...
%% Ratemap analyses          
            ...
            % accumulate data
            sdata.(uci).(part_now).field_count = length(fieldd.fields(:,1)); % add data to structure
...
%% Overdispersion analysis     
            ...
            poxmap2 = fillmissing(poxmap(uindx),'nearest');
            poymap2 = fillmissing(poymap(uindx),'nearest');            
            pspt2 = unique(pspt);
            d2 = diff(ppot2)/2;
...
%% Spike phase analyses   
            ...
            % accumulate data
            sdata.(uci).(part_now).spikes_time_histogram = [bstvals(:),bspikes(:)]; % add data to structure
            sdata.(uci).(part_now).spikes_time_ksdensity = [xibs(:),bsprobs(:)]; % add data to structure
...
% phase precession analyses taken from Jeewajee et al. (2013) Theta phase precession of grid and place cell firing in open environments  
%             ppdata = phasePRECESS2D(sdata,uci,part_now);
            
            % accumulate data
%             sdata.(uci).(part_now).phase_precession = ppdata; % add data to structure            
...
% from Kropff, Carmichael, Moser and Moser (2015) Speed cells in the medial entorhinal cortex
            ...
            sdata.(uci).(part_now).speed_curve = [xvals bvals];
            sdata.(uci).(part_now).speed_curve_normalised = [xvals bvals2];
...
%% Figures
            ...
            if run_figCLUST
                figCLUST2(sdata,uci,part_now,fig_vis,save_fig); % overall figure with ratemaps etc
            end 
            
            save([pwd '\klustest\' cname '\' cname '_sdata.mat'],'-struct','sdata','-append'); % save session data
        end 
